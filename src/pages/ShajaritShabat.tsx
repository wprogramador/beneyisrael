...
import { ArrowLeft, ChevronUp, BookOpen, Sun } from 'lucide-react'
import { Link } from 'react-router'
import { seccionesShajaritShabat } from '@/data/shajarit-shabat'

export default function ShajaritShabat() {
  const [seccionActiva, setSeccionActiva] = useState(seccionesShajaritShabat[0].id)
    ...
    <div className="min-h-screen bg-[#0c0a07] text-foreground">
      ...
      <header className="border-b border-[#d4af37]/20 bg-[#0c0a07]/95 backdrop-blur-md sticky top-0 z-40">
        <div className="mx-auto max-w-6xl px-4 sm:px-5 flex h-14 items-center justify-between">
          <Link to="/biblioteca" className="flex items-center gap-2 text-[#d4af37] hover:text-[#e9c65a] transition-colors text-sm">
            <ArrowLeft size={18} />
            Volver a Biblioteca
          </Link>
          ...
          <div className="lg:sticky lg:top-20">
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-1">
                <Sun className="size-5 text-[#d4af37]" />
                <h1 className="text-lg font-bold text-[#f5f0e6]">Shajarit Shabat</h1>
              </div>
              <p className="text-xs text-[#a89b8c]">Oración de la Mañana del Shabat — Hebreo · Fonética · Español</p>
            </div>

            <nav className="flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-visible pb-2 lg:pb-0">
              {seccionesShajaritShabat.map((sec) => (
                <button
                  key={sec.id}
            ...
            <div className="mt-6 hidden lg:block rounded-lg border border-[#d4af37]/15 bg-[#141009] p-4">
              <div className="flex items-center gap-2 text-[#d4af37] mb-2">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider">Siddur Sefardí</span>
              </div>
              <p className="text-xs text-[#a89b8c]">
                Adaptado en estilo sefardí con fonética según la pronunciación de las comunidades sefardíes.
              </p>
            </div>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0">
          <div className="mb-10 text-center lg:text-left">
            ...
            <p className="text-[#a89b8c] max-w-xl">
              Sidur de la oración de la mañana de Shabat en hebreo original, transliteración fonética
              y traducción al español. Preparado por el Centro de Estudios Bene Israel.
            </p>
          </div>

          {seccionesShajaritShabat.map((seccion) => (
          ...
          <div className="mt-12 text-center border-t border-[#d4af37]/10 pt-8">
            <p className="text-xs text-[#8a7e72] mb-2">
              Shajarit Shabat preparado por el Centro de Estudios Hebraicos Bene Israel — Los Teques, Venezuela
            </p>
