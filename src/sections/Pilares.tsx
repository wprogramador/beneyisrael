...
const pilares = [
  { icon: BookOpen, hebrew: 'תורה', titulo: 'Estudio de la Torá', texto: 'Nos dedicamos a la enseñanza y profundización en las Escrituras, buscando la verdad en el texto original y su aplicación práctica para la vida diaria.' },
  { icon: Sparkles, hebrew: 'תרבות עברית', titulo: 'Cultura Hebrea', texto: 'Fomentamos el amor por nuestras raíces, enseñando el idioma hebreo, las tradiciones, la música y la danza, para que cada miembro se sienta conectado con su herencia.' },
  { icon: HeartHandshake, hebrew: 'גמילות חסדים', titulo: 'Acción Benéfica', texto: 'Entendemos que la fe sin obras está muerta. Por eso nos esforzamos en ser una luz para nuestro entorno mediante actividades benéficas y de servicio a la comunidad en Los Teques.' },
]
    ...
    <section id="pilares" ref={ref} className="relative py-16 sm:py-24 md:py-32 bg-[#100d08]">
      <div className="mx-auto max-w-6xl px-4 sm:px-5">
        ...
        <div className="mt-16 grid md:grid-cols-3 gap-6">
          {pilares.map((p, i) => (
            <article key={p.titulo} className={`reveal ${i === 1 ? 'reveal-delay-1' : i === 2 ? 'reveal-delay-2' : ''} group relative rounded-2xl border border-[#d4af37]/20 bg-gradient-to-b from-[#1a150d] to-[#100d08] p-6 sm:p-8 hover:border-[#d4af37]/60 transition-colors duration-500`}>
        ...
        <div className="reveal mt-14">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {[
              { es: 'Idioma Hebreo', he: 'לשון הקודש', desc: 'Hebreo bíblico y moderno: la lengua viva de nuestro pueblo.' },
              ...
              <article key={a.es} className={`reveal ${i === 1 ? 'reveal-delay-1' : i === 2 ? 'reveal-delay-2' : i === 3 ? 'reveal-delay-3' : ''} rounded-2xl border border-[#d4af37]/25 bg-[#0c0a07] p-6 text-center hover:border-[#d4af37]/60 transition-colors duration-500`}>
        ...
        <div className="reveal mt-16 overflow-hidden rounded-2xl border border-[#d4af37]/20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 border-t border-[#d4af37]/20 bg-[#100d08]">
            {[
              ...
              <div key={item.label} className={`flex items-center gap-3 px-6 py-5 sm:px-5 sm:py-6 md:px-8 ${i > 0 ? 'border-t sm:border-t-0 sm:border-l border-[#d4af37]/15' : ''}`}>
